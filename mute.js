...
module.exports = {
    ...
    async execute(message, args) {
        ...
        try {
            ...
            let data = { nick: User.nickname ? "[MUTADO] " + User.nickname : "[MUTADO] " + User.user.username, roles: [muteRoleId] }

            await User.edit(data, reason)

            let embed = new Discord.MessageEmbed()
                .setTitle("🔕 Punição Registrada | Administração 🔕")
                .setColor("#7f0000")
                .setTimestamp()
                .addField("👥 Usuário:", user, true)
                .addField("🔖 Mutado por:", "<@" + message.author.id + ">",true)
